import React from 'react';
import Badge from './Badge';

/**
 * BadgeGroup component for displaying a list of badges
 */
function BadgeGroup({ children }) {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', margin: '1rem 0' }}>
      {children}
    </div>
  );
}

/**
 * Pre-configured badge components
 */
export function QQBadge({ group, link, description }) {
  return (
    <Badge
      service="badge"
      label="QQ群"
      value={group}
      color="54B4EF"
    //   logo="qq"
      logo="image/png;base64,"
      link={link || `https://qm.qq.com/cgi-bin/qm/qr?k=your-qq-group-key&jump_from=webapi&authKey=your-auth-key`}
      description={description || "点击加入"}
    />
  );
}

export function DiscordBadge({ serverId, link, description }) {
  return (
    <Badge
      service="discord"
      value={serverId}
      label=""
      logo="discord"
      logoColor="ffffff"
      color="7389D8"
      labelColor="6A7EC2"
      link={link || `https://discord.com/invite/${serverId}`}
      description={description || "点击加入"}
    />
  );
}

export function GitHubBadge({ username, link, description, repo }) {
  const value = repo ? `${username}/${repo}` : username;
  return (
    <Badge
      service="badge"
      label="GitHub"
      value={value}
      color="181717"
      logo="github"
      logoColor="white"
      style="plastic"
      link={link || `https://github.com/${username}`}
      description={description || "主页"}
    />
  );
}

// 导出所有组件
export { BadgeGroup };
export default BadgeGroup;
